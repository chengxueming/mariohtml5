...
Bird.LevelState.prototype.Enter = function() {
    var levelGenerator = new Bird.LevelGenerator(320, 15), i = 0, scrollSpeed = 0, w = 0, h = 0, bgLevelGenerator = null;
    this.Level = levelGenerator.CreateLevel(this.LevelType, this.LevelDifficulty);

    //play music here
    Bird.PlayOvergroundMusic();

    this.Paused = false;
    this.Layer = new Bird.LevelRenderer(this.Level, 320, 240);
    ...
    this.FontShadow = Bird.SpriteCuts.CreateBlackFont();
    this.Font = Bird.SpriteCuts.CreateWhiteFont();

    for (i = 0; i < 2; i++) {
        scrollSpeed = 4 >> i;
        w = ((((this.Level.Width * 16) - 320) / scrollSpeed) | 0) + 320;
        h = ((((this.Level.Height * 16) - 240) / scrollSpeed) | 0) + 240;
        bgLevelGenerator = new Bird.BackgroundGenerator(w / 32 + 1, h / 32 + 1, i === 0, this.LevelType);
        this.BgLayer[i] = new Bird.BackgroundRenderer(bgLevelGenerator.CreateLevel(), 320, 240, scrollSpeed);
    }

    Bird.MarioCharacter.Initialize(this);

    this.Sprites.Add(Bird.MarioCharacter);
    this.StartTime = 1;
    this.TimeLeft = 200;
...
Bird.LevelState.prototype.Update = function(delta) {
    ...
    this.Camera.X = Bird.MarioCharacter.X - 160;
    if (this.Camera.X < 0) {
        this.Camera.X = 0;
    }
    ...
    for (i = 0; i < this.Sprites.Objects.length; i++) {
        ...
        if (sprite !== Bird.MarioCharacter) {
            xd = sprite.X - this.Camera.X;
            yd = sprite.Y - this.Camera.Y;
            if (xd < -64 || xd > 320 + 64 || yd < -64 || yd > 240 + 64) {
                this.Sprites.RemoveAt(i);
            } else {
                if (sprite instanceof Bird.Fireball) {
    ...
    if (this.Paused) {
        for (i = 0; i < this.Sprites.Objects.length; i++) {
            if (this.Sprites.Objects[i] === Bird.MarioCharacter) {
        ...
        for (x = ((this.Camera.X / 16) | 0) - 1; x <= (((this.Camera.X + this.Layer.Width) / 16) | 0) + 1; x++) {
            for (y = ((this.Camera.Y / 16) | 0) - 1; y <= (((this.Camera.Y + this.Layer.Height) / 16) | 0) + 1; y++) {
                ...
                if (x * 16 + 8 > Bird.MarioCharacter.X + 16) {
                    dir = -1;
                }
                if (x * 16 + 8 < Bird.MarioCharacter.X - 16) {
                    dir = 1;
                ...
                if (dir !== 0) {
                    ...
                    if (((Bird.Tile.Behaviors[b & 0xff]) & Bird.Tile.Animated) > 0) {
                        if ((((b % 16) / 4) | 0) === 3 && ((b / 16) | 0) === 0) {
                            if ((this.Tick - x * 2) % 100 === 0) {
                                xCannon = x;
                                for (i = 0; i < 8; i++) {
                                    this.AddSprite(new Bird.Sparkle(this, x * 16 + 8, y * 16 + ((Math.random() * 16) | 0), Math.random() * dir, 0, 0, 1, 5));
                                }
                                this.AddSprite(new Bird.BulletBill(this, x * 16 + 8 + dir * 8, y * 16 + 15, dir));
                                hasShotCannon = true;
                            }
                        }
                    }
            ...
            for (j = 0; j < this.Sprites.Objects.length; j++) {
                if (this.Sprites.Objects[j] !== this.ShellsToCheck[i] && !this.ShellsToCheck[i].Dead) {
                    if (this.Sprites.Objects[j].ShellCollideCheck(this.ShellsToCheck[i])) {
                        if (Bird.MarioCharacter.Carried === this.ShellsToCheck[i] && !this.ShellsToCheck[i].Dead) {
                            Bird.MarioCharacter.Carried = null;
    ...
    this.Camera.X = (Bird.MarioCharacter.XOld + (Bird.MarioCharacter.X - Bird.MarioCharacter.XOld) * delta) - 160;
    this.Camera.Y = (Bird.MarioCharacter.YOld + (Bird.MarioCharacter.Y - Bird.MarioCharacter.YOld) * delta) - 120;
};
...
Bird.LevelState.prototype.Draw = function(context) {
    ...
    this.Layer.DrawExit0(context, this.Camera, Bird.MarioCharacter.WinTime === 0);
    ...
    this.DrawStringShadow(context, "Bird " + Bird.MarioCharacter.Lives, 0, 0);
    this.DrawStringShadow(context, "00000000", 0, 1);
    this.DrawStringShadow(context, "COIN", 14, 0);
    this.DrawStringShadow(context, " " + Bird.MarioCharacter.Coins, 14, 1);
    this.DrawStringShadow(context, "WORLD", 24, 0);
    this.DrawStringShadow(context, " " + Bird.MarioCharacter.LevelString, 24, 1);
    ...
    if (Bird.MarioCharacter.WinTime > 0) {
    	Bird.StopMusic();
        t = Bird.MarioCharacter.WinTime + this.Delta;
        t = t * t * 0.2;

        if (t > 900) {
            //TODO: goto map state with level won
			Bird.GlobalMapState.LevelWon();
			this.GotoMapState = true;
        }

        this.RenderBlackout(context, ((Bird.MarioCharacter.XDeathPos - this.Camera.X) | 0), ((Bird.MarioCharacter.YDeathPos - this.Camera.Y) | 0), (320 - t) | 0);
    }

    if (Bird.MarioCharacter.DeathTime > 0) {
    	Bird.StopMusic();
        t = Bird.MarioCharacter.DeathTime + this.Delta;
        t = t * t * 0.1;

        if (t > 900) {
            //TODO: goto map with level lost
			Bird.MarioCharacter.Lives--;
			this.GotoMapState = true;
			if (Bird.MarioCharacter.Lives <= 0) {
...
Bird.LevelState.prototype.DrawStringShadow = function(context, string, x, y) {
...
Bird.LevelState.prototype.RenderBlackout = function(context, x, y, radius) {
    if (radius > 320) {
        return;
    }
...
Bird.LevelState.prototype.Bump = function(x, y, canBreakBricks) {
    var block = this.Level.GetBlock(x, y), xx = 0, yy = 0;

    if ((Bird.Tile.Behaviors[block & 0xff] & Bird.Tile.Bumpable) > 0) {
        this.BumpInto(x, y - 1);
        this.Level.SetBlock(x, y, 4);
        this.Level.SetBlockData(x, y, 4);

        if ((Bird.Tile.Behaviors[block & 0xff] & Bird.Tile.Special) > 0) {
            Enjine.Resources.PlaySound("sprout");
            if (!Bird.MarioCharacter.Large) {
                this.AddSprite(new Bird.Mushroom(this, x * 16 + 8, y * 16 + 8));
            } else {
                this.AddSprite(new Bird.FireFlower(this, x * 16 + 8, y * 16 + 8));
            }
        } else {
            Bird.MarioCharacter.GetCoin();
            Enjine.Resources.PlaySound("coin");
            this.AddSprite(new Bird.CoinAnim(this, x, y));
        }
    }

    if ((Bird.Tile.Behaviors[block & 0xff] & Bird.Tile.Breakable) > 0) {
        this.BumpInto(x, y - 1);
        if (canBreakBricks) {
            ...
            for (xx = 0; xx < 2; xx++) {
                for (yy = 0; yy < 2; yy++) {
                    this.AddSprite(new Bird.Particle(this, x * 16 + xx * 8 + 4, y * 16 + yy * 8 + 4, (xx * 2 - 1) * 4, (yy * 2 - 1) * 4 - 8));
                }
            }
        }
    }
...
Bird.LevelState.prototype.BumpInto = function(x, y) {
    var block = this.Level.GetBlock(x, y), i = 0;
    if (((Bird.Tile.Behaviors[block & 0xff]) & Bird.Tile.PickUpable) > 0) {
        Bird.MarioCharacter.GetCoin();
        Enjine.Resources.PlaySound("coin");
        this.Level.SetBlock(x, y, 0);
        this.AddSprite(new Bird.CoinAnim(x, y + 1));
    }
...
Bird.LevelState.prototype.CheckForChange = function(context) {
	if (this.GotoLoseState) {
		...
		if (this.GotoMapState) {
			context.ChangeState(Bird.GlobalMapState);
		}
	}
};
