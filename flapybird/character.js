...
Bird.Character.prototype.Move = function() {
    ...
    if (Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.A) && this.CanShoot && this.Fire && this.World.FireballsOnScreen < 2) {
        ...
        this.World.AddSprite(new Bird.Fireball(this.World, this.X + this.Facing * 6, this.Y - 20, this.Facing));
    }
    ...
    if (this.Sliding) {
        this.World.AddSprite(new Bird.Sparkle(this.World, ((this.X + Math.random() * 4 - 2) | 0) + this.Facing * 8,
...
Bird.Character.prototype.CalcPic = function() {
    ...
    if (this.OnGround && ((this.Facing === -1 && this.Xa > 0) || (this.Facing === 1 && this.Xa < 0))) {
        ...
        if (this.Xa > 3 || this.Xa < -3) {
            for (i = 0; i < 3; i++) {
                this.World.AddSprite(new Bird.Sparkle(this.World, (this.X + Math.random() * 8 - 4) | 0, (this.Y + Math.random() * 4) | 0, Math.random() * 2 - 1, Math.random() * -1, 0, 1, 5));
            }
        }
    }
...
Bird.Character.prototype.IsBlocking = function(x, y, xa, ya) {
    ...
    if (((Bird.Tile.Behaviors[block & 0xff]) & Bird.Tile.PickUpable) > 0) {
        ...
        for (xx = 0; xx < 2; xx++) {
            for (yy = 0; yy < 2; yy++) {
                this.World.AddSprite(new Bird.Sparkle(this.World, x * 16 + xx * 8 + ((Math.random() * 8) | 0), y * 16 + yy * 8 + ((Math.random() * 8) | 0), 0, 0, 0, 2, 5));
            }
        }
    }
...
Bird.Character.prototype.Stomp = function(object) {
    var targetY = 0;
    ...
    if (object instanceof Bird.Enemy || object instanceof Bird.BulletBill) {
    ...
    } else if (object instanceof Bird.Shell) {
...
Bird.Character.prototype.GetHurt = function() {
    if (this.DeathTime > 0 || this.World.Paused) {
        return;
    }
    if (this.InvulnerableTime > 0) {
...
Bird.Character.prototype.Win = function() {
...
Bird.Character.prototype.Die = function() {
    this.XDeathPos = this.X | 0;
    this.YDeathPos = this.Y | 0;
    this.World.Paused = true;
    this.DeathTime = 1;
